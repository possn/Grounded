...
const initial={profile:null,startDate:null,entries:{},floods:[],activeTab:'home',breath:null};
let state=load();
...
function entry(d=todayKey()){return state.entries[d]||(state.entries[d]={date:d,morningDone:false,middayDone:false,nightDone:false,sliders:{energia:5,calma:5,esperanca:5,presenca:5,reatividade:5,proximidade:5},vivo:{},rel:{},victory:'',notes:'',intention:''})}
const missions=[
...
function rel(){const e=entry();const items=['Conversa significativa','Contacto espontâneo','Afeto','Tempo juntos sem tensão','Discussão','Reparação após discussão','Evitei pressionar','Validei antes de explicar'];return `<section class="card hero"><div class="eyebrow">Noite</div><div class="h1">Rever sem ruminar</div><p class="p">Não avalies se a relação está salva. Regista sinais observáveis.</p></section><section class="card"><div class="grid">${items.map(i=>check('rel',i,e.rel?.[i])).join('')}</div>${slider('proximidade','Proximidade sentida',e.sliders.proximidade)}${slider('esperanca','Esperança realista',e.sliders.esperanca)}<label><b>Pequena vitória do dia</b><textarea id="victory" placeholder="Ex: rimo-nos 2 minutos; não escalei a discussão; fiz algo por mim">${e.victory||''}</textarea></label><label><b>Nota curta</b><textarea id="notes" placeholder="Máximo 3 linhas. Não transformar em ruminação.">${e.notes||''}</textarea></label><button class="btn" id="saveRel">Guardar noite</button></section>`}
